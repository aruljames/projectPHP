...
class Block {
	    ...
	    if(is_array($block) && !empty($block)){
	        if(isset($block['class'])){
	            ...
	            foreach($codePools as $codePool){
	                $file_path = _ROOT . DS . 'app' . DS . 'code' . DS . 'admin' . DS . $codePool . DS . 'view' . DS . 'template' . DS . $template_path . '.php';
	                if (file_exists($file_path)) {
	                    include($file_path);return;
	                }
	            }
	            foreach($codePools as $codePool){
	                $file_path = _ROOT . DS . 'app' . DS . 'design' . DS . 'admin' . DS . $codePool . DS . 'template' . DS . $template_path . '.php';
	                if (file_exists($file_path)) {
	                    include($file_path);return;
	                }
	            }
	...
	public function getLayout($layout = null){
            ...
            return $this->_layout = new \system\admin\Layout($layout);
	}
